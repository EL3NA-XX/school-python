...
def punten (speelveld, positie = None, pad = None):
    if positie is None:
    ...
    if not opties:
        print("PAD:", pad, "score:", punten_op_positie(speelveld, positie))
        return punten_op_positie(speelveld, positie)

    beste_score = 0
    for optie in opties:
        nieuwe_positie = beweeg(positie, optie)
        nieuwe_score = punten(speelveld, nieuwe_positie, pad + [optie])
